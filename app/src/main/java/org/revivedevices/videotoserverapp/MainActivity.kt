package org.revivedevices.videotoserverapp
...
class MainActivity : AppCompatActivity() {
    ...
    /* MissingPermissions suppressed because the permissions are checked in the checkPermissions method and not granted permissions are requested by requestPermissons*/
    ...
    private fun recordVideo(videoRecordingFilePath: String) {
        camera_view.startRecording(
            getVideoOutputFileOptions(videoRecordingFilePath),
    ...
    private fun captureImage(imageCaptureFilePath: String) {
        camera_view.takePicture(
            getImageOutputFileOptions(imageCaptureFilePath),
            ContextCompat.getMainExecutor(this),
    ...
    @SuppressLint("UnsafeExperimentalUsageError")
    private fun getVideoOutputFileOptions(videoRecordingFilePath: String): OutputFileOptions {
        val videoOutputFileOptions = OutputFileOptions.builder(File(videoRecordingFilePath)).build()
        return videoOutputFileOptions
    }

    private fun getImageOutputFileOptions(imageCaptureFilePath: String): ImageCapture.OutputFileOptions {
        return ImageCapture.OutputFileOptions.Builder(File(imageCaptureFilePath)).build()
    }
